...
[![Platform](https://img.shields.io/badge/平台-macOS%20%7C%20Windows%20%7C%20Linux-lightgrey)](https://github.com/CatVinci-Studio/PaperwithAgent/releases)

---

## 这是什么？

PaperwithAgent 是一个面向研究者的桌面工具。数据不会被锁在私有数据库中——每篇论文对应磁盘上一个 Markdown 文件（YAML 元数据 + 笔记正文），并派生出 CSV 索引和 `schema.json` 自定义列定义。
...
从 [Releases](https://github.com/CatVinci-Studio/PaperwithAgent/releases) 下载对应平台的安装包。
...
cd PaperwithAgent
pnpm install
pnpm dev          # 以开发模式启动（Electron）
```

**生产构建：**

```bash
pnpm build        # 编译渲染进程 + 主进程
pnpm dist:mac     # 打包 macOS DMG
pnpm dist:win     # 打包 Windows 安装包
pnpm dist:linux   # 打包 Linux AppImage
```
...
1. **打开应用** — 首次启动时会在 `~/PaperwithAgent` 创建默认库。
