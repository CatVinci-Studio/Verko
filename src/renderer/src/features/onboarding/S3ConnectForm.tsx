  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-[var(--bg-base)] px-6 py-10 overflow-auto">
      <div className="max-w-[520px] w-full space-y-5">
        <button
          onClick={onCancel}
          className="flex items-center gap-1.5 text-[14.5px] text-[var(--text-muted)] hover:text-[var(--text-primary)]"
        >
          <ArrowLeft size={12} /> {t('common.cancel')}
        </button>

        <div>
          <h2 className="text-[18px] font-semibold text-[var(--text-primary)]">{t('welcome.s3.title')}</h2>
          <p className="text-[14.5px] text-[var(--text-muted)] mt-1">{t('welcome.s3.description')}</p>
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {PRESETS.map((p) => (
                <button
                  ...
                  className={
                    'px-2.5 py-1 rounded-[8px] text-[14px] border transition-colors ' +
                    (preset === p.id
          ...
          <label className="flex items-center gap-2 text-[14.5px] text-[var(--text-muted)]">
            <input
              type="checkbox"
...
function Field(props: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <label className="text-[14px] font-medium text-[var(--text-secondary)]">{props.label}</label>
      {props.children}
      {props.hint && <div className="text-[13px] text-[var(--text-muted)]">{props.hint}</div>}
    </div>
  )
}
