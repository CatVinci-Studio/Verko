...
export function registerLibraryHandlers(
  ...
  ipc.handle('libraries:list', async () => manager.list())

  ipc.handle('libraries:hasNone', async () => !manager.hasActive())

  ipc.handle('libraries:open', async (_, id: string) => {
    try {
      const info = await manager.markActive(id)
  ...
  ipc.handle('libraries:probeLocal', async (_, path: string): Promise<ProbeResult> => {
    try {
      ...
      if (!st.isDirectory()) return { status: 'error', message: 'Path is not a folder' }
    } catch {
  ...
  ipc.handle('libraries:exportZip', async (_, id: string): Promise<string | null> => {
    ...
    const backend = manager.buildBackend(id)
    await exportLibraryZip(backend, res.filePath)
    return res.filePath
  })
  ...
  ipc.handle('libraries:s3Creds', async (_, id: string) => manager.s3Creds(id))
