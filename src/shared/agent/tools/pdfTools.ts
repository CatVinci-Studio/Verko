...
/**
 * PDF tools — runtime-neutral.
 *
 * `extract_pdf_text` runs in any JS environment that can load `pdfjs-dist`.
 * `view_pdf_page` rasterizes via `OffscreenCanvas`, so it requires a browser
 * runtime — which is fine because the agent loop now lives in the renderer
 * for both web and desktop builds.
 */
    ...
    async call(args, { library }) {
      if (typeof OffscreenCanvas === 'undefined') {
        return JSON.stringify({
          error: 'view_pdf_page requires a browser runtime (OffscreenCanvas). The desktop build should override this tool.',
        })
      }
      const id = args['id'] as string
