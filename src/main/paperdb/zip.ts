 ...
 *
 * The archive contains every file the backend knows about — paper markdown,
 * attachments, schema.md, papers.csv, collections.json — preserving the
 * library's relative path structure. Importing the archive into an empty
 * directory reproduces the library byte-for-byte.
 */
export async function exportLibraryZip(library: Library, outPath: string): Promise<void> {
  const zip = new JSZip()
  const all = await library.backend.listFiles('')
  for (const rel of all) {
    const data = await library.backend.readFile(rel)
    zip.file(rel, data)
  }
